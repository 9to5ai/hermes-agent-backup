...
**For single-file HTML decks, use reveal.js from CDN** — it is the standard choice for browser-based presentations. Do NOT default to React for a deck unless the user specifically wants a React app.

```html
<!-- reveal.js CDN setup (minimal) -->
...
<script>Reveal.initialize({ hash: true, slideNumber: 'c/t', width: 1280, height: 720 });</script>
```

Use React for decks only when: the deck needs significant interactivity beyond slide navigation, or the deliverable must be embedded in a React codebase.
