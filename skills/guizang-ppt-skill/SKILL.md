...
### Step 1.5 · 从现有 PPTX 提取内容（用 python-pptx）

如果用户提供了 `.pptx` 文件，先把文字内容提取出来再开始：

```bash
# 检查 python-pptx 是否可用
python3 -c "from pptx import Presentation; print('pptx available')" 2>/dev/null || pip install python-pptx
...
for i, slide in enumerate(prs.slides):
    ...
    print()
EOF /path/to/deck.pptx > deck_text.md
```

**注意**：deck_text.md 是临时提取物，用于生成参考；每次应该用最新版 PPTX（文件名含版本号如 `v6`）的最新文件。

### Step 1.6 · 确认主题色

从 5 套预设里选一套（见 `references/themes.md`），告知用户选择的理由。

### Step 1.7 · 部署到 Vercel（生成完毕后）

静态 HTML 无需 npm build，直接上传：

```bash
cd ~/Projects/<project-name>
# 确保 index.html 在项目根目录，或者在 vercel.json 里配置 rewrite
zsh -lc 'vercel --prod --yes'
```

如果 `index.html` 在子目录（如 `ppt/`），在项目根目录加 `vercel.json`：
```json
